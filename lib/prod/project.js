...
{
	var _this, user, addInfo, layoutId; 
	...
	$('#layoutList').on('click','a', function()
	{
		...
		addInfo = '%user=' + user + '?%type=' + $(this).attr('data-type') + '?%layoutId=' + layoutId;
		var redirectPageNow = new Redirect('canvas', 'canvas/index.html', addInfo); // (rootDir, pageRef, addInfo
	});
};
...
$(function(){
    ...
    $.ajax(
    {
        ...
        success: function(data)
        {
            ...
            var listProducts = new ElementBuilder('tabLinks', 'productList', obj, false, false);
        }
    });
