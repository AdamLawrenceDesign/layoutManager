...
LayoutSettings.prototype.keywords = function()
{
	...
	$(node.keywordsInput).keydown(function(event)
	{
...
LayoutSettings.prototype.commit = function()
{
	...
	$(node.commit).on('click', function()
	{
		...
		editedBy = window.location.href.slice(window.location.href.search('userName=')+9, window.location.href.search('&end'));
...
LayoutSettings.prototype.delete = function()
{
	...
	$(node.delete).on('click', function()
	{
		...
		var putRequest = new ServerRequest(path, 'PUT', data,  function(data)
		{
			console.log(data);
			$(node.wrapSettings).slideUp(400, function()
				{
